new (function() {
	...
	ext.when_pin_pressed_released = function (pin, val) 
    {
        if (pin === '' || pin < 0 || pin > 27) return;
		...
		if (data.slice(0,1) == "1" && val == 'pressed') return true;
		else if (data.slice(0,1) == "0" && val == 'released') return true;
		else return false;
		
    };
    
    ext.when_pir = function (pin, val) 
    {
        if (pin === '' || pin < 0 || pin > 27) return;
		...
		if (data.slice(0,1) == "1" && val == 'motion') return true;
		else if (data.slice(0,1) == "0" && val == 'no motion') return true;
		else return false;
    ...
    ext.get_gpio_pressed_released = function (pin) 
    {
        if (pin === '' || pin < 0 || pin > 27) return;
		...
		if (data.slice(0,1) == "1") return 'pressed';
		else return 'released';
    };
    
    ext.get_gpio_motion_no_motion = function (pin) 
    {
        if (pin === '' || pin < 0 || pin > 27) return;
    ...
    if (cpu.indexOf ("ARM") != -1)
    {
        var descriptor = {
            blocks: [
				['h', 'when button on pin %m.gpios is %m.pressed_released', 'when_pin_pressed_released', ''],
				['r', 'button on pin %m.gpios', 'get_gpio_pressed_released', ''],
				['b', 'button on pin %m.gpios is %m.pressed_released?', 'when_pin_pressed_released', ''],
				['h', 'when PIR on pin %m.gpios detects %m.motion_no_motion', 'when_pir', ''],
				['r', 'PIR on pin %m.gpios', 'get_gpio_motion_no_motion', ''],
				['b', 'PIR on pin %m.gpios has detected %m.motion_no_motion?', 'when_pir', ''],
				[' ', 'turn LED on pin %m.gpios %m.on_off', 'gpio_on_off', '', 'on'],
				[' ', 'toggle LED on pin %m.gpios', 'gpio_toggle', '', 'on'],
				['r', 'LED on pin %m.gpios', 'get_gpio_on_off', ''],
				[' ', 'turn buzzer on pin %m.gpios %m.on_off', 'gpio_on_off', '', 'on'],
				[' ', 'toggle buzzer on pin %m.gpios', 'gpio_toggle', '', 'on'],
				['r', 'buzzer on pin %m.gpios', 'get_gpio_on_off', ''],
                
            ],
            menus: {
				pressed_released: ['pressed', 'released'],
				motion_no_motion: ['motion', 'no motion'],
				on_off: ['on', 'off'],
