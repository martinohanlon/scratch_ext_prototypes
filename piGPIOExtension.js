new (function() {
	...
	ext.when_pin_high_low = function (pin, val) 
    {
        if (pin === '' || pin < 0 || pin > 27) return;
		...
		if (data.slice(0,1) == "1" && val == 'high') return true;
		else if (data.slice(0,1) == "0" && val == 'low') return true;
		else return false;
    ...
    if (cpu.indexOf ("ARM") != -1)
    {
        var descriptor = {
            blocks: [
                ...
                ['h', 'when gpio %m.gpios goes %m.high_low', 'when_pin_high_low', ''],
                [' ', 'set gpio %m.gpios to %m.pull_up_down', '', '']
            ],
            menus: {
				pull_up_down: ['pull up', 'pull down'],
